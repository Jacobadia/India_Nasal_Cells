...
# signatures <- c("MS4A1", "GPR27","KDM5D","DDX3Y",
#                 "TXLNGY","PRRX2","EPOR","NOX1",
#                 "LINC00278","PLCL1","UTY","PRKY",
#                 "SPC24","RPS4Y1","ZFY","NLGN4Y","EIF1AY")
...
##### getting test data, validating from another nasal study from uganda
#### Modelling dataframe ####
...
# Convert to matrix
u_counts_data <- as.matrix(u_counts_data)
all(rownames(u_coldata) %in% colnames(u_counts_data))
all(rownames(u_coldata) == colnames(u_counts_data))
all(colnames(u_counts_data) %in% rownames(u_coldata))
all(colnames(u_counts_data) == rownames(u_coldata))

u_batch <- factor(u_coldata$sex)
u_sample_group <- factor(u_coldata$status)
u_counts_corrected <- ComBat_seq(as.matrix(u_counts_data),
                               batch=u_batch,
                               group=u_sample_group)


# DESeq2
dds <- DESeqDataSetFromMatrix(countData = u_counts_corrected,
                              colData = u_coldata,
                              design = ~ status)
...
rownames(vst_mat)
colnames(vst_mat)
all(colnames(vst_mat) == rownames(u_coldata))
...
train_data <- make_model_data(sig_genes_present)
...
ggsave("models/roc_plots_figure_6_external.pdf", plot = roc_grid, width = 12, height = 8)
