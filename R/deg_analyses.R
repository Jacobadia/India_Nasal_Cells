...
# protein_mean_10_control_nothing <- function(matrix_counts_file, metadata_file, gene_type_file) {
#     dir_name <- "protein_mean_10_control_nothing"
#     full_dir_path <- paste0(artifacts_dir, dir_name)
#     dir.create(full_dir_path, showWarnings = FALSE)
#     full_deg_results_file <- paste0(full_dir_path, "/deg_results_full.txt")
#     significant_deg_results_file <- paste0(full_dir_path, "/deg_results_significant.txt")
...
protein_mean_100_control_nothing <- function(matrix_counts_file, metadata_file, gene_type_file) {
    dir_name <- "protein_mean_100_control_nothing"
    full_dir_path <- paste0(artifacts_dir, dir_name)
    dir.create(full_dir_path, showWarnings = FALSE)
    full_deg_results_file <- paste0(full_dir_path, "/deg_results_full.txt")
    significant_deg_results_file <- paste0(full_dir_path, "/deg_results_significant.txt")
    counts <- get_protein_coding_non_zero(matrix_counts_file, gene_type_file)
    counts <- filter_mean_counts(counts, 100)
    conditionData <- get_condition_data(counts, metadata_file)
    run_deg_control_nothing(counts, conditionData, 
    full_deg_results_file, significant_deg_results_file)
    create_pvalue_histogram(dir_name, full_deg_results_file)
...
protein_mean_10_control_sex <- function(matrix_counts_file, metadata_file, gene_type_file) {
    dir_name <- "protein_mean_10_control_sex"
    full_dir_path <- paste0(artifacts_dir, dir_name)
    dir.create(full_dir_path, showWarnings = FALSE)
    full_deg_results_file <- paste0(full_dir_path, "/deg_results_full.txt")
    significant_deg_results_file <- paste0(full_dir_path, "/deg_results_significant.txt")
    ...
    counts <- filter_mean_counts(counts, 10)
    conditionData <- get_condition_data(counts, metadata_file)
    run_deg_control_sex(counts, conditionData, 
    full_deg_results_file, significant_deg_results_file)
    create_pvalue_histogram(dir_name, full_deg_results_file)
}

protein_mean_100_control_sex <- function(matrix_counts_file, metadata_file, gene_type_file) {
    dir_name <- "protein_mean_100_control_sex"
    full_dir_path <- paste0(artifacts_dir, dir_name)
    dir.create(full_dir_path, showWarnings = FALSE)
    full_deg_results_file <- paste0(full_dir_path, "/deg_results_full.txt")
    significant_deg_results_file <- paste0(full_dir_path, "/deg_results_significant.txt")
...
# protein_mean_100_control_nothing(matrix_counts_file, metadata_file, gene_type_file)
# protein_mean_10_control_sex(matrix_counts_file, metadata_file, gene_type_file)
# protein_mean_100_control_sex(matrix_counts_file, metadata_file, gene_type_file)
